...
  echo "-p, 
port: The Jetty HTTP port to use. Overrides any value from JETTY_PORT. Defaults to 8080."
  echo "-sp, 
stopport: The Jetty stop port to use. Overrides any value from JETTY_STOP_PORT. Defaults to 8079."
  echo "-ld, 
lockdir: The directory where the executing process id is stored to verify that that only one instance"
  echo "    is started. Defaults to /var/tmp."
  echo "-j, 
jmx: Allows monitoring/managing Jetty through JMX."
  echo "-ni, 
noninteractive: Don't ask questions to the user. Useful when called in an automated script."
  echo ""
...
  # remove 
 for Cygwin
  local lines=$("$java_cmd" -Xms32M -Xmx32M -version 2>&1 | tr '
' '
')
  if 
 -z $java_cmd 
...
      if 
 (-z $result) && ($line = *"version ""*) 
; then
        local ver=$(echo $line | sed -e 's/.*version "\(.*\)"\(.*\)//; 1q')
        # on macOS, sed doesn't support '?'
        if 
"* 
; then
          result=$(echo $ver | sed -e 's/1\.\([0-9]*\)\(.*\)//; 1q')
        else
          result=$(echo $ver | sed -e 's/\([0-9]*\)\(.*\)//; 1q')
        fi
      fi
    done
  fi
...
# TODO: Remove once 
https://jira.xwiki.org/browse/XCOMMONS-2852
 is fixed. In summary we need this to allow the XWiki
# code or 3rd party code to use reflection to access private variables (setAccessible() calls).
# See 
https://tinyurl.com/tdhkn6mp
